...
class MessageLinkButton(urwid.Button):
    ...
    def _patch_narrow_link(self, parsed_link: ParsedNarrowLink) -> None:
        ...
        if "stream" in parsed_link:
            ...
            pm_type: Literal["pm", "group"] = (
                "pm" if len(recipient_ids) < 3 else "group"
            )

            # Patch the PM type and recipient emails
            parsed_link["pm_with"]["type"] = pm_type
