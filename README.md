...
* **Latest** - The latest version can be installed from the main development version from git
...
With the required python 3.5+, the following should work on most systems:
...
Upon first running `zulip-term`, it looks for a `zuliprc` file, by default in your home directory.
...
2. Each time you run `zulip-term`, you can specify the path to an alternative `zuliprc` file using the `-c` or `--config-file` options, eg. `$ zulip-term -c /path/to/zuliprc`
   ...
   A personal zuliprc file can be obtained from Zulip servers in your account settings, which gives you all the permissions you have in the web app. Bot zuliprc files can be downloaded from the area for each bot, and will have more limited permissions.
...
[zterm]
# Alternative themes can be found using the `--list-themes` option
theme=default
